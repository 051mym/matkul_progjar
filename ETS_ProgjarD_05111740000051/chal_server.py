...
while True:
    try:
        ...
        elif data_msg.split()[2] == "SEND":
            recv_file(data_msg.split()[3])
        elif data_msg.split()[2] == "UPTRACT":
            recv_file(data_msg.split()[3].replace('/','-') + '.zip')
        ...
        for client in set_client_addr:
            ...
            if msg_usr != user:
                ...
                if data_msg.split()[2] == "SEND":
                    send_file(data_msg.split()[3])
                if data_msg.split()[2] == "UPTRACT":
                    send_file(data_msg.split()[3].replace('/','-') +'.zip')
